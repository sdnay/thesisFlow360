...
interface SimpleTaskTagManagerProps {
  availableTags: Tag[];
  selectedTags: Tag[];
  onTagAdd: (tagNameOrTag: string | Tag) => Promise<void>; // Can be async if creating tags
  onTagRemove: (tagId: string) => void;
  disabled?: boolean;
}

const SimpleTaskTagManager: FC<SimpleTaskTagManagerProps> = ({
  availableTags,
  selectedTags,
  onTagAdd,
  onTagRemove,
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap gap-1 min-h-[24px]"> {/* Min height for consistent layout */}
      ...
      <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
        <PopoverTrigger asChild>
          <Input
            ...
            disabled={disabled}
            aria-label="Ajouter un tag pour la tâche"
          />
        </PopoverTrigger>
          ...
          <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0" align="start">
            <Command>
              <CommandInput 
                ...
                className="h-8 text-xs" // Smaller input in popover
              />
              <CommandList>
                <CommandEmpty>
                  {inputValue.trim() ? (
...
export default SimpleTaskTagManager;
