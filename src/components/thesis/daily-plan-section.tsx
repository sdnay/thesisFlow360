...
import TagManager from '@/components/ui/tag-manager'; // **** NOUVEL IMPORT ****
...
export function DailyPlanSection() {
  ...
  const fetchPageData = useCallback(async (dateToFetch: Date) => {
    ...
    try {
      ...
      setObjectives((objRes.data || []).map(o => ({...o, tags: o.daily_objective_tags?.map((dot:any) => dot.tags) || [] })));
      setChapters(chapRes.data || []);
      setAvailableTags(tagsRes.data || []);
      setDailyPlanForSelectedDate(dailyPlanRes.data as DailyPlan | null);
      setOverdueObjectives((overdueResOrNull?.data || []).map(o => ({...o, tags: o.daily_objective_tags?.map((dot:any) => dot.tags) || [] })));
  ...
  const handleAddOrUpdateObjective = async () => {
    ...
    try {
      const objectivePayload = {
        ...
        chapter_id: selectedChapterId || null, daily_plan_id: dailyPlanForSelectedDate?.id || null,
      };
      let savedObjectiveData: Pick<DailyObjective, 'id'>;

      if (editingObjective) {
        const { data, error } = await supabase.from('daily_objectives').update(objectivePayload).eq('id', editingObjective.id).select('id').single();
        if (error) throw error; savedObjectiveData = data!;
      } else {
        const { data, error } = await supabase.from('daily_objectives').insert(objectivePayload).select('id').single();
        if (error) throw error; savedObjectiveData = data!;
      }
      ...
      const processedFinalObjective: DailyObjective = { ...fetchedObjective, tags: fetchedObjective.daily_objective_tags?.map((dot: any) => dot.tags) || [] };

      if (formatDateForSupabase(selectedDate) === processedFinalObjective.objective_date) {
          if (editingObjective) setObjectives(prev => prev.map(obj => obj.id === processedFinalObjective.id ? processedFinalObjective : obj).sort((a,b) => new Date(a.created_at!).getTime() - new Date(b.created_at!).getTime()));
          else setObjectives(prev => [...prev, processedFinalObjective].sort((a,b) => new Date(a.created_at!).getTime() - new Date(b.created_at!).getTime()));
      }
  ...
  const handleToggleObjective = async (id: string, completed: boolean, objectiveDate: string) => {
    ...
    try {
      ...
      if (updatedObj) {
          const processedUpdatedObj: DailyObjective = { ...updatedObj, tags: updatedObj.daily_objective_tags?.map((dot: any) => dot.tags) || [] };
          if (objectiveDate === formatDateForSupabase(selectedDate)) {
            ...
            if (isSelectedDateToday && completed) setOverdueObjectives(prevOverdue => prevOverdue.filter(obj => obj.id !== id));
          } else {
             if(completed) setOverdueObjectives(prevOverdue => prevOverdue.filter(obj => obj.id !== id));
             else fetchPageData(selectedDate); 
          }
      }
    } catch (e: any) { toast({title:"Erreur", description: (e as Error).message, variant:"destructive"});
  ...
  const handleAddTagToManualObjective = async (tagOrNewName: Tag | string) => {
    ...
    if (typeof tagOrNewName === 'string') {
      ...
      if (existingTag) finalTag = existingTag;
        ...
        finalTag = newTagFromDb; setAvailableTags(prev => [...prev, finalTag!].sort((a,b)=>a.name.localeCompare(b.name)));
      }
  ...
  const handleSaveDailyPlan = async (title: string | null) => {
    ...
    try {
        const planPayload = { user_id: user.id, plan_date: formatDateForSupabase(selectedDate), title: title || null, id: dailyPlanForSelectedDate?.id || undefined };
        const { data: savedPlan, error } = await supabase.from('daily_plans').upsert(planPayload).select().single();
        if (error) throw error;
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 h-full flex flex-col">
      ...
      <Card className="shadow-sm"><CardHeader className="pb-2 pt-4 flex flex-row items-center justify-between"><CardTitle className="text-base md:text-lg">Progression ({format(selectedDate, "d MMMM", { locale: fr })})</CardTitle></CardHeader><CardContent className="py-3"><Progress value={completionPercentage} className="w-full h-2.5 mb-1" /><p className="text-xs text-muted-foreground text-center">{completionPercentage}% ({completedCount}/{totalForSelectedDate})</p></CardContent></Card>
      ...
      <div className="flex-grow space-y-3 overflow-y-auto custom-scrollbar pr-1 pb-4 min-h-[200px]">
        {isFetching ? (<div className="flex-grow flex flex-col items-center justify-center text-center p-6"><Loader2 className="h-10 w-10 animate-spin text-primary mb-4" /><p className="text-muted-foreground">Chargement...</p></div>
      ...
      <Dialog open={isModalOpen} onOpenChange={(open) => {if(!isFormLoading) setIsModalOpen(open)}}>
        <DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle className="text-lg">{editingObjective ? "Modifier l'Objectif" : `Nouvel Objectif (${format(selectedDate, "d MMM", { locale: fr })})`}</DialogTitle></DialogHeader>
            <div className="py-4 space-y-4">
                 ...
                 <div><Label htmlFor="objectiveChapterSelect" className="mb-1.5 block text-sm">Chapitre (Optionnel)</Label><Select value={selectedChapterId || "none"} onValueChange={(value) => setSelectedChapterId(value === "none" ? undefined : value)} disabled={isFormLoading || chapters.length === 0 || !user}><SelectTrigger id="objectiveChapterSelect" className="text-sm h-10"><SelectValue placeholder={chapters.length === 0 ? "Aucun chapitre" : "Lier à un chapitre..."} /></SelectTrigger><SelectContent><SelectItem value="none">Aucun chapitre</SelectItem>{chapters.map(chap => <SelectItem key={chap.id} value={chap.id}>{chap.name}</SelectItem>)}</SelectContent></Select></div>
                 <div><Label className="mb-1.5 block text-sm">Tags (Optionnel)</Label><TagManager availableTags={availableTags} selectedTags={selectedTags} onTagAdd={handleAddTagToManualObjective} onTagRemove={handleRemoveTagFromManualObjective} disabled={isFormLoading || !user} isLoading={isFetching} triggerLabel="Gérer les tags de l'objectif"/></div>
            ...
            <DialogFooter className="mt-2"><DialogClose asChild><Button variant="outline" disabled={isFormLoading}>Annuler</Button></DialogClose><Button onClick={handleAddOrUpdateObjective} disabled={isFormLoading || !user || !newObjectiveText.trim() || (objectives.filter(obj => !obj.completed && obj.objective_date === formatDateForSupabase(selectedDate)).length >= 5 && !editingObjective && isSelectedDateToday)}><Loader2 className={isFormLoading ? "h-4 w-4 animate-spin mr-2" : "hidden"} />{editingObjective ? <Save className="h-4 w-4 mr-2" /> : <PlusCircle className="h-4 w-4 mr-2" />}{editingObjective ? 'Enregistrer' : 'Ajouter'}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isPlanModalOpen} onOpenChange={setIsPlanModalOpen}>
        <DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle className="text-lg">{dailyPlanForSelectedDate?.id ? "Modifier le Titre du Plan" : `Créer un Plan pour le ${format(selectedDate, "d MMM", { locale: fr })}`}</DialogTitle></DialogHeader>
            <CreateDailyPlanForm planDate={selectedDate} userId={user?.id || ''} existingPlan={dailyPlanForSelectedDate} onSubmitPlan={handleSaveDailyPlan} isSubmitting={isSavingPlan} onCancel={() => setIsPlanModalOpen(false)}/>
        </DialogContent>
      </Dialog>
    </div>
  );
